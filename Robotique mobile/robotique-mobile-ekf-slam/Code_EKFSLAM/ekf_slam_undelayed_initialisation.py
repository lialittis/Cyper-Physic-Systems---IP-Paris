...
# Calculate the landmark position based on the necessary information
def calc_landmark_position(x, y, nLandmarks, delta):
    """
    Computes absolute landmark position from robot pose and observation
    """

    yp = np.zeros((nLandmarks*2, 1))

    for i in range(nLandmarks):
        yp[0+2*i, 0] = x[0, 0] + delta*(i+1)*math.cos(x[2, 0] + y[1])
        yp[1+2*i, 0] = x[1, 0] + delta*(i+1)*math.sin(x[2, 0] + y[1])
...
def calc_innovation(xEst, PEst, y, LMid):
    ...
    # We will only use the information of direction
    innov = np.array(innov[1],ndmin=2)
    H = H[1:2,:]
    S = H @ PEst @ H.T + Py[1,1]
    return innov, S, H


def ekf_slam(xEst, PEst, u, y, ld_marks):
    """
    ...
    for iy in range(len(y[:, 0])):  # for each observation
        ...
        if min_id == nLM:
            ...
            nLandmarks = 8 # possible landmarks
            delta = 1 # distance 
            ...
            xEst = np.vstack((xEst, calc_landmark_position(xEst, y[iy, :],nLandmarks,delta)))

            for i in range(nLandmarks):

                ld_marks = ld_marks + [1] # add one possible landmark
                
                y_p = y[iy,:]
                # print(y,y_p)
                y_p[0] = (i+1) * delta
                Jr, Jy = jacob_augment(xEst[0:3], y_p)
            ...
            # print("min_id is",min_id)
            ld_marks[min_id] = ld_marks[min_id] + 1
            x = np.array(ld_marks)
            ...
            sum_freq = np.sum(np.exp(x))
            ld_freq = np.exp(x) / sum_freq
            # ld_freq = x / np.sum(x)
            # print(ld_freq)
            #remove_signal = np.zeros(len(ld_freq),dtype=bool)

            threshold = 0.01
            
            for i in range(len(ld_freq)) :
                if ld_freq[i] < threshold:
                    # remove_signal[i] = True
                    # print("remove_signal",remove_signal)
                    xEst = np.delete(xEst, STATE_SIZE + 2*i, 0)
                    xEst = np.delete(xEst, STATE_SIZE + 2*i, 0)
                    PEst = np.delete(PEst, STATE_SIZE + 2*i, 0)
                    PEst = np.delete(PEst, STATE_SIZE + 2*i, 0)
                    PEst = np.delete(PEst, STATE_SIZE + 2*i, 1)
                    PEst = np.delete(PEst, STATE_SIZE + 2*i, 1)
                    del(ld_marks[i])
                    break
...
def main():
    ...
    while  time <= SIM_TIME:
        ...
        xEst, PEst, ld_marks = ekf_slam(xEst, PEst, u, y,ld_marks )
        ...
        if show_animation and count%1==0:
            ...
            plt.savefig('undelayed_slam' + str(count) + '.png')
            plt.pause(0.1)
